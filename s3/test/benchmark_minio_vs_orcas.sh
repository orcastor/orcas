...
# Build the benchmark server from s3/test/benchmark_server.go
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"

# Build from project root - now s3 directory only has package s3, no conflict
cd "$PROJECT_ROOT"
go build -o "$SCRIPT_DIR/orcas_benchmark_server" \
    -ldflags="-s -w" \
    ./s3/test/benchmark_server.go 2>&1
BUILD_STATUS=$?
