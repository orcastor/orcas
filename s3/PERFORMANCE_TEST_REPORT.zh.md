...
| PutObject | 4KB | 850.32 | 320.15 | 1.18ms / 3.12ms | 3.32 / 1.25 |
| PutObject | 64KB | 295.50 | 265.50 | 3.38ms / 3.77ms | 18.47 / 16.59 |

**分析：**
- 小文件（1KB）：**930.65 ops/sec（批量写入开启）** vs **331.98 ops/sec（批量写入关闭）**（**提升180.4%**）
- 小文件（4KB）：**850.32 ops/sec（批量写入开启）** vs **320.15 ops/sec（批量写入关闭）**（**提升165.6%**）
- 小文件（64KB）：**295.50 ops/sec（批量写入开启）** vs **265.50 ops/sec（批量写入关闭）**（**提升11.3%**，64KB是批量写入阈值上限，仍可使用批量写入）
...
**4KB文件（小文件）：**
- **10并发**：1580.25 ops/sec（批量写入开启）vs 853.42 ops/sec（批量写入关闭）= **提升85.2%**
- **50并发**：1350.68 ops/sec（批量写入开启）vs 853.12 ops/sec（批量写入关闭）= **提升58.3%**
- **100并发**：1420.35 ops/sec（批量写入开启）vs 876.45 ops/sec（批量写入关闭）= **提升62.1%**

**关键发现：**
- **小文件（1KB）**：批量写入在所有并发级别下提供**66-99%的性能提升**
- **小文件（4KB）**：批量写入在所有并发级别下提供**58-85%的性能提升**
