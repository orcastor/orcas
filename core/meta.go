...
// key: encryption key (deprecated, no longer used for database encryption)
func InitDB(basePath, key string) error {
	...
	// Note: key parameter is no longer used for database connections
	db, err := GetWriteDB(basePath)
	if err != nil {
		...
		// Note: Don't close the connection, it's from the pool or should be managed by caller
	}
...
func InitBucketDB(dataPath string, bktID int64, key ...string) error {
	...
	// Note: key parameter is deprecated and no longer used for database encryption
...
// Note: This is kept for API compatibility but not used for database connections anymore
// Database encryption is no longer used; only name encryption via dataDBKey is supported
func (dba *DefaultBaseMetadataAdapter) SetBaseKey(key string) {
	// No-op: dbKey is no longer stored or used
}
...
func (dma *DefaultMetadataAdapter) decryptObjNames(objs []*ObjectInfo) error {
	...
	for _, obj := range objs {
		...
		if dma.DefaultDataMetadataAdapter.aead == nil {
			// File name is encrypted but no decryption key available
			// This can happen if:
			// 1. User mounted without providing DataDBKey
			// 2. User provided wrong DataDBKey
			// Return error to alert user
			return fmt.Errorf("file name is encrypted but decryption key not available (objID=%d, encrypted name length=%d)", obj.ID, len(obj.Name))
		}
...
func (dma *DefaultMetadataAdapter) UpdateBktQuota(c Ctx, bktID int64, quota int64) error {
	...
	db, err := GetWriteDB(bktDirPath)
	if err != nil {
		return fmt.Errorf("%w: UpdateBktQuota GetWriteDB failed (bktID=%d, path=%s): %v", ERR_OPEN_DB, bktID, bktDirPath, err)
	}
	// Note: Don't close the connection, it's from the pool

	// Create table context and check for nil
	table := b.TableContext(c, db, BKT_TBL)
	if table == nil {
		return fmt.Errorf("%w: UpdateBktQuota TableContext returned nil (bktID=%d)", ERR_EXEC_DB, bktID)
	}
...
func (dma *DefaultMetadataAdapter) IncBktRealUsed(c Ctx, bktID int64, size int64) error {
	...
	updateBucketStatsCache(bktID, dma.DefaultDataMetadataAdapter.dataPath, 0, size, 0, 0)
	return nil
}
...
func (dma *DefaultMetadataAdapter) IncBktUsed(c Ctx, bktID int64, size int64) error {
	// 使用ecache异步合并刷新，不立即写入数据库
	updateBucketStatsCache(bktID, dma.DefaultDataMetadataAdapter.dataPath, size, 0, 0, 0)
	return nil
}
...
func (dma *DefaultMetadataAdapter) IncBktLogicalUsed(c Ctx, bktID int64, size int64) error {
	// 使用ecache异步合并刷新，不立即写入数据库
	updateBucketStatsCache(bktID, dma.DefaultDataMetadataAdapter.dataPath, 0, 0, size, 0)
	return nil
}
...
func (dma *DefaultMetadataAdapter) IncBktDedupSavings(c Ctx, bktID int64, size int64) error {
	// 使用ecache异步合并刷新，不立即写入数据库
	updateBucketStatsCache(bktID, dma.DefaultDataMetadataAdapter.dataPath, 0, 0, 0, size)
	return nil
}
