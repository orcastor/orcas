...
func TestCompleteWritingVersion(t *testing.T) {
	Convey("CompleteWritingVersion", t, func() {
		ig := idgen.NewIDGen(nil, 0)
		testBktID, _ := ig.New()
		InitBucketDB(c, testBktID)
		...
		Convey("complete writing version", func() {
			// Create a file first
			fileID, _ := ig.New()
			fileObj := &ObjectInfo{
				ID:     fileID,
			...
			// Complete writing version
			err = lh.CompleteWritingVersion(c, testBktID, fileID)
			So(err, ShouldBeNil)

			// Verify version name changed
			versions, err := lh.ma.ListVersions(c, testBktID, fileID, false)
			So(err, ShouldBeNil)
			So(len(versions), ShouldEqual, 1)
			So(versions[0].Name, ShouldNotEqual, WritingVersionName)
			So(versions[0].Name, ShouldNotEqual, "")
			// Name should be a timestamp string
			So(versions[0].MTime, ShouldBeGreaterThan, writingVersion.MTime)
		})

		Convey("complete non-existent writing version", func() {
			// Create a file first
			fileID, _ := ig.New()
			fileObj := &ObjectInfo{
				ID:     fileID,
			...
			// Try to complete non-existent writing version
			err = lh.CompleteWritingVersion(c, testBktID, fileID)
