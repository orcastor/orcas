...
func (fs *OrcasFS) cleanupInactiveRandomAccessors() {
	...
	// MEMORY OPTIMIZATION: Process cleanup with limited concurrency using worker pool
	// This prevents AES decryption temporary buffers from accumulating
	const maxConcurrentCleanup = 5 // Limit concurrent cleanup operations

	if len(inactiveFileIDs) == 0 {
		return
	}

	// Use worker pool pattern to safely limit concurrency
	// This avoids channel deadlock/close issues by using a task queue and worker goroutines
	taskChan := make(chan int64, len(inactiveFileIDs))
	var wg sync.WaitGroup
	var cleanedCount int64

	// Start worker goroutines
	for i := 0; i < maxConcurrentCleanup; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			for fileID := range taskChan {
				if val, ok := fs.raRegistry.Load(fileID); ok {
					if ra, ok := val.(*RandomAccessor); ok && ra != nil {
						// Try to close the RandomAccessor to release resources
						// Note: Close() may trigger ForceFlush which reads/decrypts data
				...
				fs.raRegistry.Delete(fileID)
				atomic.AddInt64(&cleanedCount, 1)
			}
		}()
	}
	...
	close(taskChan) // Safe to close: all tasks are sent, workers will finish processing

	// Wait for all workers to complete
	wg.Wait()
	cleanedCountInt := int(atomic.LoadInt64(&cleanedCount))

	if cleanedCountInt > 0 {
		DebugLog("[VFS RA Registry] Cleanup completed: total=%d, cleaned=%d, remaining=%d",
			totalBefore, cleanedCountInt, totalBefore-cleanedCountInt)
		...
		if cleanedCountInt >= 5 {
			DebugLog("[VFS RA Registry] Forcing GC after cleanup of %d RandomAccessors", cleanedCountInt)
			runtime.GC()
		}
	}
}
