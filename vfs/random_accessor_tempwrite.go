...
// shouldCompressFileByName determines if a file should be compressed based on its name
// This is a wrapper around the existing shouldCompressFile function
...
func (ra *RandomAccessor) shouldUseTempWriteArea(fileObj *core.ObjectInfo) bool {
	...
	if fileObj.Size >= ra.fs.tempWriteArea.config.SmallFileThreshold {
		DebugLog("[VFS shouldUseTempWriteArea] Large file: fileID=%d, size=%d, threshold=%d", 
			ra.fileID, fileObj.Size, ra.fs.tempWriteArea.config.SmallFileThreshold)
		return true
	}
...
func (ra *RandomAccessor) getOrCreateTempWriteFile() (*TempWriteFile, error) {
	...
	needsCompress := shouldCompressFileByName(fileObj.Name)
